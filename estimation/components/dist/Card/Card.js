...
const transform = scale => props => {
  if (props.setPose === 'selected') return `scale(1.1) translateX(0px) translateY(-20px) rotate(0deg)`;
  if (props.setPose === 'choosen') return `scale(1.5) translateX(0px) translateY(-60px) rotate(0deg)`;
  if (props.setPose === 'hidden') return `scale(1) translateX(150px) translateY(-250px) rotate(0deg)`;
  let deg = 0;
  let translateX = 0;
...
const Card = (0, _styledComponents.default)(_reactPose.default.button({
  hoverable: true,
  pressable: true,
  init: {
